...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  paper2code-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  paper2code-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"paper2code-enhanced-server-health", "runs", "stats"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  paper2code-pp-cli workflow status

  # Show status as JSON
  paper2code-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("paper2code-pp-cli")
			}
