...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PAPER2CODE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PAPER2CODE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "paper2code-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.paper2code-pp-cli/feedback.jsonl.
When ` + "`PAPER2CODE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PAPER2CODE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  paper2code-pp-cli feedback list
  paper2code-pp-cli feedback list --limit 5
  paper2code-pp-cli feedback list --json`,
