...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "paper2code-pp-cli",
		Short: "Manage paper2code resources via the paper2code API",
		Long: `Manage paper2code resources via the paper2code API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'paper2code-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("paper2code-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'paper2code-pp-cli profile list')")
	...
	rootCmd.AddCommand(newEvalPromotedCmd(flags))
	rootCmd.AddCommand(newPaper2codeEnhancedServerHealthPromotedCmd(flags))
	rootCmd.AddCommand(newPdfPromotedCmd(flags))
	rootCmd.AddCommand(newPipelinePromotedCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("paper2code-pp-cli %s\n", version)
		},
	}
}
