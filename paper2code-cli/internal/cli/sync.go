...
Resource scoping:
  ...
  paper2code-pp-cli sync

  # Sync specific resources only
  paper2code-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  paper2code-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  paper2code-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  paper2code-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  paper2code-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/paper2code-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"paper2code-enhanced-server-health": "/health",
		"runs":                              "/runs",
		"stats":                             "/stats",
	}
