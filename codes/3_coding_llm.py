...
parser.add_argument(
    "--model_name", type=str, default="deepseek-ai/DeepSeek-Coder-V2-Lite-Instruct"
)
...
for todo_file_name in todo_file_lst:
    ...
    with open(
        f"{output_dir}/{save_todo_file_name}_simple_analysis_trajectories.json",
        encoding="utf8",
    ) as f:
        detailed_logic_analysis_trajectories = json.load(f)

    detailed_logic_analysis_dict[todo_file_name] = detailed_logic_analysis_trajectories[
        0
    ]["content"]
...
for todo_file_name in tqdm(todo_file_lst):
    ...
    instruction_msg = get_write_msg(
        todo_file_name, detailed_logic_analysis_dict[todo_file_name], done_file_lst
    )
    ...
    # save artifacts
    with open(
        f"{artifact_output_dir}/{save_todo_file_name}_coding.txt", "w", encoding="utf-8"
    ) as f:
        f.write(completion)
