...
def cal_cost(response_json, model_name):
    ...
    # Some providers (Ollama, MiniMax, OpenRouter) omit prompt_tokens_details
    prompt_details = response_json["usage"].get("prompt_tokens_details") or {}
    ...
    # Unknown models (MiniMax, Ollama, OpenRouter, etc.) — skip cost calculation
    if model_name not in model_cost:
        print(
            f"[INFO] Cost tracking not available for model '{model_name}' — token counts logged only."
        )
        return {
    ...
    input_cost = (actual_input_tokens / 1_000_000) * (cost_info["input"] or 0.0)
    cached_input_cost = (cached_tokens / 1_000_000) * (cost_info["cached_input"] or 0.0)
    output_cost = (output_tokens / 1_000_000) * (cost_info["output"] or 0.0)
...
def print_log_cost(
    ...
    output_lines.append(
        f"📥 Input tokens: {usage_info['actual_input_tokens']} (Cost: ${usage_info['input_cost']:.8f})"
    )
    output_lines.append(
        f"📦 Cached input tokens: {usage_info['cached_tokens']} (Cost: ${usage_info['cached_input_cost']:.8f})"
    )
    output_lines.append(
        f"📤 Output tokens: {usage_info['output_tokens']} (Cost: ${usage_info['output_cost']:.8f})"
    )
    output_lines.append(f"💵 Current total cost: ${current_cost:.8f}")
    output_lines.append(
        f"🪙 Accumulated total cost so far: ${total_accumulated_cost:.8f}"
    )
    output_lines.append("============================================\n")
...
def num_tokens_from_messages(messages, model="gpt-4o-2024-08-06"):
    ...
    for message in messages:
        ...
        for key, value in message.items():
            ...
            num_tokens += len(
                encoding.encode(
                    value, allowed_special={"<|endoftext|>"}, disallowed_special=()
                )
            )
...
def read_all_files(directory, allowed_ext, is_print=True):
    ...
    for root, _, files in os.walk(directory):  # Recursively traverse directories
        for filename in files:
            ...
            if (
                filename.startswith(".")
                or "requirements.txt" in filename
                or ext == ""
                or is_skip
            ):
                if is_print and ext == "":
