...
from utils import content_to_json, extract_planning, print_response
...
for todo_file_name in tqdm(todo_file_lst):
    ...
    with open(
        f"{output_dir}/{todo_file_name}_simple_analysis_response.json",
        "w",
        encoding="utf-8",
    ) as f:
    ...
    with open(
        f"{output_dir}/{todo_file_name}_simple_analysis_trajectories.json",
        "w",
        encoding="utf-8",
    ) as f:
