...
def main(args):
    ...
    try:
        ...
        print(
            f"[WARNING] An exception was raised while counting tokens for the target repository of {args.paper_name}."
        )
        print(e)
        print("-" * 40)
        num_tokens = 0
    ...
    if reasoning_effort:
        if generated_n > 8:
            print(
                "[WARNING] Reasoning models may not support n > 8. Setting generated_n to 8."
            )
            generated_n = 8
        request_json = {
            "model": gpt_version,
    ...
    for n in range(generated_n):
        ...
        try:
            ...
            try:
                ...
                print("[WARNING] Invalid repsponse: parsing error")
                print(e2)
                print("-" * 40)
        ...
        if score < 1 or score > 5:
            print(
                f"[WARNING] Invalid repsponse: score {score}, Score must be in the range of 1–5."
            )
            continue
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--eval_type", type=str, default="ref_free", choices=["ref_free", "ref_based"]
    )
